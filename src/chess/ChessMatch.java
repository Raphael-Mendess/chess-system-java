...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (onePieceMoved instanceof Pawn) {
			if (onePieceMoved.getColor() == Color.WHITE && target.getRow() == 0
					|| onePieceMoved.getColor() == Color.BLACK && target.getRow() == 7) {
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		Piece x = board.removePiece(pos);
		
		piecesOnTheWar.remove(x);

		ChessPiece newPieceOfPromotion = newPieceOfPromotion(type, promoted.getColor());
		
		board.placePiece(newPieceOfPromotion, pos);
		piecesOnTheWar.add(newPieceOfPromotion);

		return newPieceOfPromotion;
	}

	private ChessPiece newPieceOfPromotion(String type, Color color) {
		if (type.equals("B"))
	...
	private ChessPiece kingOfBattle(Color color) {
		List<Piece> list = piecesOnTheWar.stream().filter(x -> ((ChessPiece) x).getColor() == color)
	...
	private boolean testCheckKing(Color color) {
		...
		List<Piece> opponentPieces = piecesOnTheWar.stream()
				.filter(x -> ((ChessPiece) x).getColor() == opponentRival(color)).collect(Collectors.toList());
